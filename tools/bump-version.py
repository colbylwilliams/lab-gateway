...
s_fmt = 'VERSION = \'{}\''
r_fmt = 'https://github.com/colbylwilliams/lab-gateway/releases/latest/download/lab_gateway-{}-py2.py3-none-any.whl'
h_fmt = '{}\n++++++\n{}\n\n{}'
...
with open(Path(Path.cwd() / 'client/lab-gateway') / 'setup.py', 'w') as f:
    f.write(setup)


print('..updating HISTORY.rst')

with open(Path(Path.cwd() / 'client/lab-gateway') / 'HISTORY.rst', 'r') as f:
    history = f.read()

history = history.replace(v.public, h_fmt.format(n.public, notes, v.public))

with open(Path(Path.cwd() / 'client/lab-gateway') / 'HISTORY.rst', 'w') as f:
    f.write(history)
